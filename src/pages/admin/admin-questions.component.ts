...
@Component({
  ...
  template: `
    ...
    <!-- Filter -->
    <div class="mb-6">
      <input type="text" placeholder="Buscar por enunciado..." (input)="search.set($any($event.target).value)" class="w-full max-w-md px-4 py-2 border rounded-lg">
    </div>
        ...
        <form [formGroup]="form" (ngSubmit)="submit()">
          <div class="space-y-4 mb-4">
             <div>
               ...
               <textarea formControlName="text" rows="2" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2"></textarea>
             </div>
             <div>
             ...
             <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div><input formControlName="optA" placeholder="Opção A" class="w-full border rounded p-2"></div>
               <div><input formControlName="optB" placeholder="Opção B" class="w-full border rounded p-2"></div>
               <div><input formControlName="optC" placeholder="Opção C" class="w-full border rounded p-2"></div>
               <div><input formControlName="optD" placeholder="Opção D" class="w-full border rounded p-2"></div>
             </div>

             <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div>
                  <label class="block text-sm font-medium text-gray-700">Correta</label>
                  <select formControlName="correctIndex" class="mt-1 w-full border rounded p-2">
               ...
               <div>
                 ...
                 <input formControlName="explanation" class="mt-1 w-full border rounded p-2">
               </div>
             </div>
          </div>
